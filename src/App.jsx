...
function detectCols(headers) {
  ...
  return {
    ...
    ggImp: exact("Google Ads-Imp"), ggRoas: exact("Google Ads-ROAS"), ggCr: exact("Google Ads-CR"),
    fbSpend: exact("Facebook Ads-Amount spent"), fbClicks: exact("Facebook Ads-Link clicks"),
...
function compute(rows, cols) {
  ...
  const totalGGSpend=sum(cols.ggSpend), totalFBSpend=sum(cols.fbSpend);
  const totalAdSpend=totalGGSpend+totalFBSpend;
  const totalRevenueGG=sum(cols.revenueGG), totalRevenueFB=sum(cols.revenueFB);
  const totalAdRevenue=totalRevenueGG+totalRevenueFB;
  const totalUnitsGG=sum(cols.unitsGG), totalUnitsFB=sum(cols.unitsFB);
  ...
  valid.forEach(r => {
    ...
    const p=skuMap[k];
    p.revenue+=get(r,cols.revenue); p.units+=get(r,cols.units); p.sessions+=get(r,cols.sessions);
    p.atc+=get(r,cols.atc); p.checkout+=get(r,cols.checkout);
    p.revenueGG+=get(r,cols.revenueGG); p.unitsGG+=get(r,cols.unitsGG); p.sessionsGG+=get(r,cols.sessionsGG);
    p.revenueFB+=get(r,cols.revenueFB); p.unitsFB+=get(r,cols.unitsFB); p.sessionsFB+=get(r,cols.sessionsFB);
    p.ggSpend+=get(r,cols.ggSpend); p.ggConv+=get(r,cols.ggConv); p.ggValue+=get(r,cols.ggValue);
    p.ggClick+=get(r,cols.ggClick); p.ggImp+=get(r,cols.ggImp);
    p.fbSpend+=get(r,cols.fbSpend); p.fbClicks+=get(r,cols.fbClicks); p.fbImp+=get(r,cols.fbImp);
  ...
  const quadrant=p=>{const h=p.revenue>=revMedian,m=(p.margin||0)>=0.35;return h&&m?"star":h&&!m?"cashcow":!h&&m?"gem":"risk";};

  const funnelTrafficATC=skuList.filter(p=>p.sessions>=10).sort((a,b)=>b.sessions-a.sessions).slice(0,10);
  const funnelATCCheckout=skuList.filter(p=>p.atc>=3).map(p=>({...p,checkoutRate:p.atc>0?p.checkout/p.atc:null})).sort((a,b)=>b.atc-a.atc).slice(0,10);
  const funnelCheckoutPurchase=skuList.filter(p=>p.checkout>=2).map(p=>({...p,purchaseRate:p.checkout>0?p.units/p.checkout:null})).sort((a,b)=>b.checkout-a.checkout).slice(0,10);
...
const Modal=({title,color,items,cols,onClose})=>(
  <div className="modal-overlay" onClick={onClose}>
    <div className="modal-box" onClick={e=>e.stopPropagation()}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
        <div style={{fontSize:14,fontWeight:600,color:color||T.text0}}>{title} <span style={{fontSize:12,color:T.text2,fontWeight:400}}>({items.length} products)</span></div>
      ...
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:11}}>
        <thead><tr>{cols.map(c=>(<th key={c.k} style={{textAlign:c.r?"right":"left",padding:"5px 8px",color:T.text2,fontWeight:500,borderBottom:`1px solid ${T.line1}`}}>{c.label}</th>))}</tr></thead>
        <tbody>{items.map((row,i)=>(<tr key={i} style={{background:i%2===0?"transparent":T.bg2}}>{cols.map(c=>(<td key={c.k} style={{padding:"6px 8px",borderBottom:`1px solid ${T.line1}`,textAlign:c.r?"right":"left",color:T.text0,whiteSpace:c.wrap?"normal":"nowrap",maxWidth:c.wrap?220:undefined}}>{c.render?c.render(row):row[c.k]}</td>))}</tr>))}</tbody>
      </table>
    </div>
  </div>
);
  ...
  return(
    <div style={{overflowX:"auto"}}>
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
        ...
        <tbody>{rows.slice(0,max).map((row,i)=>(<tr key={i} className="row-hover" style={{background:i%2===0?"transparent":T.bg2+"88"}}>{tcols.map(c=>(<td key={c.k} style={{padding:"7px 10px",borderBottom:`1px solid ${T.line1}`,textAlign:c.r?"right":"left",color:c.color?c.color(row):T.text0,whiteSpace:c.wrap?"normal":"nowrap",maxWidth:c.wrap?200:undefined}}>{c.render?c.render(row):row[c.k]}</td>))}</tr>))}</tbody>
      </table>
    </div>
  );
};
...
const FunnelViz=({sessions,atc,checkout,units})=>{
  ...
  return(<div style={{display:"flex",flexDirection:"column",gap:8}}>{steps.map((s,i)=>{const drop=i>0?((steps[i-1].value-s.value)/steps[i-1].value):0;return(<div key={i}><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}><div style={{width:90,fontSize:11,color:T.text2}}>{s.label}</div><HBar value={s.value} max={max} color={s.color} h={8}/><div style={{width:60,textAlign:"right",fontSize:12,fontFamily:MONO,color:T.text0}}>{fNum(s.value)}</div>{i>0&&<div style={{width:50,textAlign:"right",fontSize:11,color:drop>0.5?T.red:T.text2}}>-{fPct(drop)}</div>}</div></div>);})}</div>);
};

const QuadCard=({label,count,items})=>{
  const [showModal,setShowModal]=useState(false);
  const map={star:{icon:"⭐ Star",color:T.green,desc:"High revenue + high margin"},cashcow:{icon:"💰 Cash Cow",color:T.yellow,desc:"High revenue + low margin"},gem:{icon:"💎 Hidden Gem",color:T.cyan,desc:"Low revenue + high margin"},risk:{icon:"⚠️ At Risk",color:T.red,desc:"Low revenue + low margin"}};
  const {icon,color,desc}=map[label];
  return(
    <>
      <div style={{background:T.bg2,border:`1px solid ${T.line1}`,borderLeft:`3px solid ${color}`,borderRadius:"0 10px 10px 0",padding:"12px 14px",minHeight:200}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
          <div><div style={{fontSize:12,fontWeight:600,color}}>{icon}</div><div style={{fontSize:11,color:T.text2}}>{desc}</div></div>
          <div style={{fontSize:22,fontWeight:700,fontFamily:MONO,color:T.text0}}>{count}</div>
        </div>
        <div style={{fontSize:10,color:T.text2,display:"grid",gridTemplateColumns:"1fr 55px 45px 35px",gap:4,marginBottom:4,padding:"0 2px"}}>
          <span>Product</span><span style={{textAlign:"right"}}>Revenue</span><span style={{textAlign:"right"}}>Margin</span><span style={{textAlign:"right"}}>Units</span>
        </div>
        <div style={{display:"flex",flexDirection:"column",gap:2}}>
          {items.slice(0,5).map((p,i)=>(
            <div key={i} style={{display:"grid",gridTemplateColumns:"1fr 55px 45px 35px",gap:4,padding:"3px 2px",borderBottom:`1px solid ${T.line1}`}}>
              <span style={{fontSize:11,color:T.text1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{p.name||p.sku}</span>
              <span style={{fontSize:11,color:T.text2,textAlign:"right",fontFamily:MONO}}>{fUsd(p.revenue)}</span>
              <span style={{fontSize:11,color:T.text2,textAlign:"right",fontFamily:MONO}}>{fPct(p.margin)}</span>
              <span style={{fontSize:11,color:T.text2,textAlign:"right",fontFamily:MONO}}>{fNum(p.units)}</span>
            </div>
          ))}
        </div>
      ...
      {showModal&&(<Modal title={icon} color={color} items={items} cols={[{k:"name",label:"Product",wrap:true},{k:"revenue",label:"Revenue",r:true,render:p=>fUsd(p.revenue)},{k:"units",label:"Units",r:true,render:p=>fNum(p.units)},{k:"margin",label:"Margin",r:true,render:p=>fPct(p.margin)}]} onClose={()=>setShowModal(false)}/>)}
    </>
  );
};
...
const StockCard=({label,color,count,items})=>{
  ...
  return(
    <>
      ...
      {showModal&&(<Modal title={label} color={color} items={items} cols={[{k:"name",label:"Product",wrap:true},{k:"stock",label:"Stock",r:true,render:p=>fNum(p.stock)},{k:"revenue",label:"Revenue",r:true,render:p=>fUsd(p.revenue)},{k:"units",label:"Units Sold",r:true,render:p=>fNum(p.units)}]} onClose={()=>setShowModal(false)}/>)}
    </>
  );
};
...
const FunnelLeakTable=({title,tooltip,items,col1,col1label,rateKey,rateLabel,empty})=>(
  <Card title={title} icon="ti-alert-triangle" sub={empty} tooltip={tooltip}>
    {items.length?(
      <div style={{display:"flex",flexDirection:"column",gap:3}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 50px 40px 50px 55px",gap:4,padding:"0 4px 4px",borderBottom:`1px solid ${T.line1}`}}>
        ...
        {items.map((p,i)=>(
          <div key={i} style={{display:"grid",gridTemplateColumns:"1fr 50px 40px 50px 55px",gap:4,padding:"5px 4px",borderRadius:6,background:i%2===0?"transparent":T.bg2+"88"}}>
            ...
            <div style={{fontSize:11,fontWeight:600,color:rateColor(p[rateKey]),textAlign:"right"}}>{fPct(p[rateKey])}</div>
...
const TabAds=({m})=>{
  ...
  const ggScale=[...m.skuList].filter(p=>p.ggSpend>0&&p.beRoas&&p.ggRoas>=p.beRoas&&p.sessionsGG>=50&&p.unitsGG>=2&&p.ggConv>=2&&p.stock>5&&(p.margin||0)>=0.35).sort((a,b)=>b.ggRoas-a.ggRoas).slice(0,10);
  const fbScale=[...m.skuList].filter(p=>p.fbSpend>0&&p.beRoas&&p.fbRoas>=p.beRoas&&p.sessionsFB>=50&&p.unitsFB>=2&&p.stock>5&&(p.margin||0)>=0.35).sort((a,b)=>b.fbRoas-a.fbRoas).slice(0,10);
  const chanSwitch=(<div style={{display:"flex",gap:6,marginBottom:10}}><SubTab label="Google Ads" active={channel==="gg"} onClick={()=>setChannel("gg")}/><SubTab label="Facebook Ads" active={channel==="fb"} onClick={()=>setChannel("fb")}/></div>);
  const ch=channel;
  const adsCols=[
    {k:"name",label:"Product",wrap:true},
  ...
  return(
    <>
            ...
            <tbody>
              {[["Spend",fUsd(m.totalGGSpend),fUsd(m.totalGGSpend),fUsd(m.totalFBSpend),fUsd(m.totalFBSpend)],
                ...
                <tr key={i} className="row-hover" style={{background:i%2===0?"transparent":T.bg2+"88"}}>
                  ...
                  {[v1,v2,v3,v4].map((v,j)=>(<td key={j} style={{padding:"7px 10px",borderBottom:`1px solid ${T.line1}`,textAlign:"right",color:v.includes?.("soon")||v==="—"?T.text2:T.text0,fontFamily:MONO,fontSize:12}}>{v}</td>))}
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <Card title="Top Products by Ads Spend" icon="ti-trophy" tooltip={`Sort: Spend DESC, Top 10`}>{chanSwitch}<Tbl max={10} cols={adsCols} rows={channel==="gg"?ggTop:fbTop}/></Card>
      <Card title="Wasted Spend" icon="ti-flame" tooltip={`Spend > 0\nROAS < Break-even ROAS (1/Margin)\nSessions ≥ 10\nSort: Spend DESC`}>
        {chanSwitch}
        <Tbl max={10} cols={[
          {k:"name",label:"Product",wrap:true},
          {k:"spend",label:"Spend",r:true,render:p=>fUsd(ch==="gg"?p.ggSpend:p.fbSpend)},
      ...
      <Card title="Scale Up Candidates" icon="ti-rocket" tooltip={`ROAS ≥ Break-even\nSessions-channel ≥ 50\nPurchases ≥ 2\nStock > 5\nMargin ≥ 35%\nSort: ROAS DESC`}>
        {chanSwitch}
        <Tbl max={10} cols={[
          {k:"name",label:"Product",wrap:true},
          {k:"spend",label:"Spend",r:true,render:p=>fUsd(ch==="gg"?p.ggSpend:p.fbSpend)},
...
const TabHealth=({m})=>{
  ...
  return(
    <>
      <Card title="Product Health Matrix" icon="ti-layout-grid" sub="Revenue vs Margin — threshold: median + 35%" tooltip={`Revenue: median of products with revenue > 0\nMargin threshold: 35%`}>
      ...
      <Card title="Ads Not Converting + High Traffic Zero Revenue" icon="ti-alert-circle" tooltip={`Group 1: GG Spend > 0 AND ROAS < BE ROAS AND Sessions-GG ≥ 10\nGroup 2: Sessions ≥ 30 AND Purchases = 0 AND Revenue = 0\nSort: Spend DESC`}>
        <Tbl max={10} cols={[
          {k:"name",label:"Product",wrap:true},
...
export default function App() {
  ...
  const handleLoad=useCallback((url,text)=>{
    try{
      ...
      setMetrics(m);setLastSync(new Date());
